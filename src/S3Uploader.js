...
const S3Uploader = () => {
  ...
  useEffect(() => {
    async function fetchBuckets() {
      try {
        const response = await axios.get('http://3.84.62.68:3000/buckets');
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(`http://3.84.62.68:3000/buckets/${selectedBucket}/upload`, formData, {
  ...
  const handleBucketChange = async (bucketName) => {
    ...
    try {
      const response = await axios.get(`http://3.84.62.68:3000/buckets/${bucketName}`);
